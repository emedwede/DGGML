...
{
    graph.addNode({1, {{8, 6, 0}, {0, 0, 0}, DGGML::Plant::positive}});
    graph.addNode({2, {{8, 5, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({3, {{5, 5, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({4, {{4, 4, 0}, {0, 0, 0}, DGGML::Plant::junction}});
    graph.addNode({5, {{3, 5, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({6, {{3, 6, 0}, {0, 0, 0}, DGGML::Plant::negative}});
    graph.addNode({7, {{3, 4, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({8, {{2, 3, 0}, {0, 0, 0}, DGGML::Plant::negative}});
    graph.addNode({9, {{1, 5, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({10, {{0, 4, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({11, {{0, 3, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({12, {{0, 1, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({13, {{1, 0, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({14, {{3, 1, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({15, {{4, 2, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({16, {{6, 3, 0}, {0, 0, 0}, DGGML::Plant::junction}});
    graph.addNode({17, {{5, 4, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({18, {{7, 4, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({19, {{9, 4, 0}, {0, 0, 0}, DGGML::Plant::positive}});
    graph.addNode({20, {{7, 2, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({21, {{9, 2, 0}, {0, 0, 0}, DGGML::Plant::negative}});
...
{
    //graph.addNode()
    graph.addNode({1, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({2, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({3, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({4, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::intermediate}});
    graph.addNode({5, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::junction}});
    graph.addNode({6, {{0, 0, 0}, {0, 0, 0}, DGGML::Plant::junction}});
    ...
    for(auto iter = graph.node_list_begin(); iter != graph.node_list_end(); iter++)
    {
        ...
        if(itype == DGGML::Plant::junction)
        {
            for(auto jter = graph.out_neighbors_begin(i); jter != graph.out_neighbors_end(i); jter++)
            {
                ...
                if(jtype == DGGML::Plant::intermediate)
                {
                        ...
                        if(k != i)
                        {
                            if(ktype == DGGML::Plant::junction)
                            {
                                         ...
                                         lter != graph.out_neighbors_end(i); lter++)
                                {
                                    ...
                                    if(l != j)
                                    {
                                        if(ltype == DGGML::Plant::intermediate)
                                        {
                                                    ...
                                                    mter != graph.out_neighbors_end(i); mter++)
                                            {
                                                ...
                                                if(m != j && m != l)
                                                {
                                                    if(mtype == DGGML::Plant::intermediate)
                                                    {
                                                                ...
                                                                nter++)
                                                        {
                                                            ...
                                                            if(n != j && n != l && n != m)
                                                            {
                                                                if(ntype == DGGML::Plant::intermediate)
                                                                {
...
TEST_CASE("Heuristic Growing End Matcher Test", "[heuristic-matcher-test]")
{
    ...
    {
        DGGML::Plant::microtubule_growing_end_polymerize_rewrite(graph, match);
    }
...
TEST_CASE("Heuristic Retraction End Matcher Test", "[heuristic-matcher-test]")
{
    ...
    auto matches = DGGML::Plant::microtubule_retraction_end_matcher(graph, bucket);
    print_matches(matches);
...
TEST_CASE("Heuristic Wildcard Matcher Test", "[heuristic-matcher-test]")
{
    ...
    auto matches = DGGML::Plant::wildcard_intermediate_wildcard_matcher(graph, bucket);
    print_matches(matches);
