...
{    
    ...
    Cajete::set_unit_vector(x2, x3, u2);

    double sol[2]; //results for intersection parameters in 2D

    double d_p = Cajete::unit_dot_product(u1, u2);

    //ensure the dot product is valid
    REQUIRE(d_p == 0.0);

    Cajete::paramaterized_intersection(x1, x2, x3, u1, sol);
    
    REQUIRE(sol[0] == 2);
    REQUIRE(sol[1] == 1.0/3.0);
    
    //order of x2 and x3 doesn't matter
    Cajete::paramaterized_intersection(x1, x3, x2, u1, sol);
    REQUIRE(sol[0] == 2);
